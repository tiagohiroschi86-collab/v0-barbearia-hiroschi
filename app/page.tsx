  ...
  useEffect(() => {
    ...
    const initApp = () => {
          ...
          btnAderirClube.addEventListener("click", async () => {
            ...
            try {
              await addDoc(collection(db, "solicitacoes_clube"), {
                ...
                plano_escolhido: planoClubeSelecionado.nome,
                valor_plano: planoClubeSelecionado.valor,
                data_solicitacao: new Date().toISOString(),
                status: "interesse",
              })
              alert(`Perfeito, ${clienteApelido}! Seu interesse no plano ${planoClubeSelecionado.nome} foi registrado.`)
              abrirMenuPrincipal()
            } catch (e) {
      ...
      async function carregarFinanceiroAdmin() {
        ...
        try {
          // 1) Soma agendamentos confirmados do dia
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .produto-foto { width: 100%; height: 150px; object-fit: cover; display: block; }
        .produto-sem-foto { display: flex; align-items: center; justify-content: center; background-color: #e4e6eb; color: #8d949e; font-size: 13px; }
